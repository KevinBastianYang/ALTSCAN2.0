...
void    zOutputSFVec(zTrellis*,zSFVec*, char*, char*, score_t*, int, zSFVSponge*);
//Added by Jim
void zOutputSFVSponge(zTrellis*, char*, char*, zSFVSponge*);
...
//	score_t    score[top_num];
	...
	size = 0;//size records the length of whole command line
	for(i = 0; i < argc; i++){
	...
	//write all arguments to full_command_line
	size = strlen(argv[0]);
	for(i = 1; i < argc; i++){
	...
	zSetProgramName(argv[0]);//if in a pipeline, easy for distinguish which made the mistake
	zParseOptions(&argc, argv);//deal with commandline with - or =
	if (zOption("top")){
		top_mode=true;
	...
	hmm_file = argv[optind];//read parameter file
	//printf ("optind after hmm: %d\n",optind);
	dna_file = argv[optind+1];//read fasta file
	/*	printf("%d\n",top_number); */
	...
	//here are various extra options to trigger various modes
	if(zOption("p")){
	...
	zSetVerbosityLevel(lib_verbosity);//MAYBE output verbosity packages

	/* read in HMM from file */
	//printf("after readin HMM: %s\n",argv[optind]);
	...
	if(optimized_mode){
		...
		sfv = zRunViterbiAndForward(&trellis,score,sponge);/*THE CORE of whole program*/
		printf("QWQQ-1\n");
		if (zOption("dump")) {
...
// printf("%d,\n",a->dna->length );
	...
	if(!zOption("top")){
		zOutputSFVec(&trellis,sfv[0],basename(dna_file),NULL,score,0,sponge);
    }
	else{
		printf("SCOUT:\n");
		if (!zOption("a"))
		...
		if (zOption("a"))
		{
	//		zOutputSFVSponge(&trellis, basename(dna_file), NULL, sponge);
		}
		else{
			for(kk=0;kk<top_num;kk++){
			////////////for(kk=0;kk<111;kk++){//temp_try by Jim
				printf("Q%d\n",kk);
				fprintf(outfile,"\n#Transcript: %d \n",(kk+1));
				sco=score+kk;
				zOutputSFVec(&trellis,sfv[kk],basename(dna_file),NULL,sco,kk,sponge);
			}			
		}
	}
...
void zOutputSFVec(zTrellis* trellis, zSFVec* sfv, char* filename, char* header, score_t* score, int t_num,zSFVSponge *sponge){
	zGTFVec gtfvec;
...
void zOutputSFVSponge(zTrellis* trellis, char* filename, char* header, zSFVSponge *sponge){
	zGTFVec gtfvec;
...
void zDumpTrellis(zTrellis* trellis, coor_t start, coor_t end) {
	...
	for (pos = start; pos<= end; pos++) {
		...
		i ++;
		for (state = 0; state < trellis->hmm->states; state++) {
			printf("%10g\t", trellis->cell[pos][state].layer[0]->score);
		}
		printf("\n");
	}
