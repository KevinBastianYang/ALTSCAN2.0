...
int zReadHMM (FILE *stream, zHMM *hmm, hmm_mode_t hmm_mode) {
	...
	//parameter header are pluged in
	...
	//printf("INSIDE zREADHMM %s LALALA%d\n",type,states);
	...
	if (strcmp(tag, "<STATES>") == 0) {
		...
		/* ISO tag absent, set up one big isochore */
			...
			//printf("Inside else of tag check\n");
			hmm->iso_states = 1;
		...
		//printf("orig_states:%d\n",hmm->orig_states);//orig_states=49
		...
		for (i = 0; i < hmm->orig_states; i++) {
			...
			if (UNDEFINED_STRAND == hmm->state[j-1].strand) {//didn't go in
				//printf("Inside undefined_strand\n");
	...
	while (zReadTag(stream, tag)) {
		//printf("Inside zreadtag\n");//go in 7 times
		if (strcmp(tag, "<STATE_TRANSITIONS>") == 0) {
			//printf("Inside tag check2\n");//go in
			while (isspace((int)(line[0] = (char)getc(stream)))); /* skip white space */
			if(fgets(line+1, sizeof(line) - 1, stream) == NULL) return 0;
			...
			if (sscanf(line, "%16s %d levels:", buf, &iso_transitions) == 2 && strcmp(buf,"ISO")==0 ) { /* ISO tag present */
				...
				//didn't go in
				//printf("Inside else of tag check3\n");
			...
			//printf("hmm_transitions:%d\n",hmm->transitions);//hmm_transitions:134
			...
			//printf("Inside else1 of tag check2\n");//2nd 4th time
			...
			//printf("Inside else2 of tag check2\n");//5th time
			...
			//printf("Inside else3 of tag check2\n");//6th time
			...
			//printf("Inside else4 of tag check2\n");//didn't go in
			char state_name[33];
			...
			//printf("Inside else5 of tag check2\n");//7th time
