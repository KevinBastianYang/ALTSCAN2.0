...
void zInitSfeature (zSfeature*f)
{
	zClearSfeature(f);
}
...
void zInitSFVec (zSFVec *vec, int limit) {
	...
	int i;
	for (i = 0; i < vec->limit; ++i)
	{
		zInitSfeature(&vec->elem[i]);
	}
}
...
void zPushSFVec (zSFVec *vec, const zSfeature *feature) {
	...
	if (vec->limit == vec->size) {
		...
		//new for storage check
	// 	int i;
	// 	zSfeature * tmp;
	// 	tmp = zMalloc(vec->limit * sizeof(zSfeature), "zPushSFVecNew");
	// 	for (i = 0; i < vec->size; ++i)
	// 	{
	// 		zCopySfeature(&vec->elem[i], &tmp[i]);
	// 	}
	// 	zFreeSFVec(vec);
	// //	zFree(vec->elem);
	// 	vec->elem=tmp;
...
void zFreeSFVec (zSFVec *vec) {
	...
	for (i = 0; i < vec->size; ++i)
	{	
	//	printf("state feature\n");
		zFreeSfeature(&vec->elem[i]);
	//	printf("finish feature\n");
	}
	zFree(vec->elem);
	// if (vec->elem==NULL)
	// {
...
//Added by Jim
void zTrimSFVec (zSFVec *vec, int trim)
{
	int i;
	if (trim<=0)
	{
	...
	if (trim >= vec->size)
	{
		zDie("zTrimSFVec Error");
	}
	// for (i = vec->size-1; i >=vec->size-trim; i--)
	// {
	// //	zFreeSfeature(&vec->elem[i]);
	// 	vec->size--;
	// }
	for (i = vec->size-trim; i < vec->size; ++i)
	{
	//	vec->elem[i]=vec->elem[i+trim];
	//	zCopySfeature(&vec->elem[i+trim], &vec->elem[i]);
		zFreeSfeature(&vec->elem[i]);
	}
	vec->size -= trim;
	vec->last = &vec->elem[vec->size-1];
}
...
//	printf("MERGE %d\n",vec->size);
	...
	for (i = 0; i < vec->size; ++i)
	{
		zPushSFVec(m_vec,&vec->elem[i]);
	}
}

void zConcatenateSFVec (zSFVec *vec, zSFVec *m_vec)
{ //concatenate vec to m_vec
//only for concatenate cpoint
//	printf("CONCATENATE %d\n",vec->size);
//	int orig_loc=m_vec->size-1;
	int orig_loc=0;
	int i_mark=-1;
	int j_mark=-1;
	int i,j;
	int flag=0;
	int trim;
	if (vec->size==0 || orig_loc<0 )
	{
		zDie("zConcatenateSFVec Size Error");
	}
// 	printf("%d,%d\n",vec->elem[0].start,vec->elem[0].end);
// 	printf("%d,%d\n",m_vec->elem[m_vec->size].start,m_vec->elem[m_vec->size].end);
	for (i = 0; i < vec->size; ++i)
	{
	//	printf("CONii: %d,%d\n",vec->elem[i].state,vec->elem[i].start );
		for (j = m_vec->size-1; j>=0; j--)
		{
	//		printf("    CONjj: %d,%d\n",m_vec->elem[j].state,m_vec->elem[j].start );
			if(//vec->elem[i].state==m_vec->elem[j].state && 
					vec->elem[i].start==m_vec->elem[j].start)
			{
				i_mark=i;
				j_mark=j;
				flag=1;
	...
	if (i_mark==-1 || j_mark==-1)
	{
		for (i = 0; i < vec->size; ++i)
		{
			printf("concaError ii: %d,%d-%d\n",vec->elem[i].state,vec->elem[i].start,vec->elem[i].end );
		}
		for (j = 0; j < m_vec->size; ++j)
		{
			printf("concaError jj: %d,%d-%d\n",m_vec->elem[j].state,m_vec->elem[j].start,m_vec->elem[j].end );
		}
		zDie("zConcatenateSFVec Rim Error");



		//for new feature ztrim
		// i_mark=0;
		// j_mark=m_vec->size;
	}
...
//	printf("CCCOOONNNN: %d,%d",vec->size-i_mark,m_vec->size-j_mark );

	trim=(m_vec->size-j_mark)-(vec->size-i_mark);
	...
	zTrimSFVec(m_vec,trim);

//	printf("NEWCCCOOONNNN: %d,%d",vec->size-i_mark,m_vec->size-j_mark );
...
//	else{printf("\n");}
	...
	for (j = j_mark; j < m_vec->size; ++j)
	{
		zCopySfeature(&vec->elem[i], &m_vec->elem[j]);
		i++;
	}



//	zCopySfeature(&vec->elem[0], &m_vec->elem[m_vec->size-1]);
// 	m_vec->elem[m_vec->size].end=vec->elem[0].end;
// 	printf("new%d,%d\n",vec->elem[0].start,vec->elem[0].end);
// 	printf("%d,%d\n",m_vec->elem[m_vec->size].start,m_vec->elem[m_vec->size].end);
...
void zPrintSfeature(zSfeature *f)
{	
	printf("%d -> %d, %d\n", f->start,f->end,f->state);
}

void zPrintSFList(zSFList* sfl)
{
	if (sfl==NULL)
	{
		printf("ZERO\n");
	...
	zSfeature* tmp;
//	printf("aaa\n");
	tmp = zSFListMoveFirst(sfl);
//	printf("bbb\n");
	while(tmp!=NULL)
	{
		zPrintSfeature(tmp);
//		printf("ccc\n");
		tmp = zSFListMoveNext(sfl);
	}
}


zSFVSpongeNode* zSFVSpongeMoveFirst(zSFVSponge* sp)
{
	sp->current = sp->head->next;
	if (sp->current == sp->tail){return NULL;}
	return sp->current;
}

zSFVSpongeNode* zSFVSpongeMoveLast(zSFVSponge* sp)
{
	sp->current = sp->tail->prev;
	if (sp->current == sp->head){return NULL;}
	return sp->current;
}
zSFVSpongeNode* zSFVSpongeMoveNext(zSFVSponge* sp)
{
	if (sp->current != sp->tail)
	{
		sp->current = sp->current->next;
		if (sp->current != sp->tail)
		{
			return sp->current;
		}
...
zSFVSpongeNode* zSFVSpongeMovePrev(zSFVSponge* sp)
{
	if (sp->current != sp->head)
	{
		sp->current = sp->current->prev;
		if (sp->current != sp->head)
		{
			return sp->current;
		}
...
zSFVSpongeNode* zSFVSpongeGetCurrent(zSFVSponge* sp)
{
	if ((sp->current == sp->head) || (sp->current == sp->tail))
	{
		return NULL;
	}
	else{
...
zSFVSpongeNode* zSFVSpongeAcquireNode()
{
	printf("inside\n");
	zSFVSpongeNode* temp;
	temp = zMalloc(sizeof(zSFVSpongeNode),"zSFVSpongeAcquireNode temp");
	temp->next = NULL;
	temp->prev = NULL;
	temp->start = 0;
	temp->end = 0;
...
void zInitSFVSponge(zSFVSponge* sp)
{
	printf("Sponge init\n");
	sp->head = zSFVSpongeAcquireNode();
	sp->tail = zSFVSpongeAcquireNode();
	sp->current = sp->head;
	sp->head->prev = NULL;
...
void zFillSFVSponge(zSFVSponge* sp,zSFVec** tmp_sfv,int limit)//,zTrellis *trellis)
{
	if (limit <= 0)
	{
		zDie("zFillSFVSponge limit error");
	}
	int i,g,data_size,elem_last;
	zSFVec** tmp_tmp_sfv;
	tmp_tmp_sfv = zCalloc(limit,sizeof(zSFVec*),"zMalloc tmp_tmp_sfv");
	for(g=0;g<limit;g++)
	{
		tmp_tmp_sfv[g]=zMalloc(sizeof(zSFVec),"zMalloc tmp_tmp_sfv[g]");
		zInitSFVec(tmp_tmp_sfv[g],10);
		zMergeSFVec(tmp_sfv[g],tmp_tmp_sfv[g]);
		zFlipSFVec(tmp_tmp_sfv[g]);
	}

	zSFVSpongeNode* temp = zSFVSpongeAcquireNode();
	temp->node = zCalloc(limit,sizeof(zSFVec*),"zFillSFVSponge temp");//not released now
	temp->score_box = zCalloc(limit,sizeof(score_t),"zFillSFVSponge score_box");//not released now
	data_size = sizeof(zSFVec);
	for (i = 0; i < limit; ++i)
	{
		temp->node[i]=zMalloc(data_size,"zFillSFVSponge node[i]");
		zInitSFVec(temp->node[i],10);
		zMergeSFVec(tmp_tmp_sfv[i],temp->node[i]);
//		temp->score_box[i]=zScoreSFVec(trellis,temp->node[i]);
	}	
	temp->size = limit;
	elem_last = temp->node[0]->size-1;
	temp->start = temp->node[0]->elem[0].start;
	temp->end = temp->node[0]->elem[elem_last].end;
	temp->end_state = temp->node[0]->elem[elem_last].state;


	// temp->start = temp->node[0]->elem[elem_last].start;
	// temp->end = temp->node[0]->elem[0].end;
	// temp->end_state = temp->node[0]->elem[0].state;
	temp->prev = sp->tail->prev;
	temp->next = sp->tail;
	temp->location = sp->size;
	sp->tail->prev->next = temp;
	sp->tail->prev = temp;
	sp->size++;
	if (temp->prev!=sp->head)
	{
		temp->pre_state = temp->prev->end_state;
	}
...
void zInsertSFVSponge(zSFVSponge* sp,zSFVec** tmp_sfv,int limit)//,zTrellis *trellis)
{
	if (limit <= 0)
	{
		zDie("zFillSFVSponge limit error");
...
void zFreeSFVSponge(zSFVSponge* sp){printf("1\n");}
