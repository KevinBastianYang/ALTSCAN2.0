...
//	printf("AAAzCalloc %s:%p~%d\n",str,buffer,nobj*size);
//	printf("zCalloc %s: %p\n",str,&buffer );
	return buffer;  
}
...
void zPushIVec (zIVec *vec, int val) {
	...
	//	vec->elem = zRealloc(vec->elem, vec->limit * sizeof(int), "zPushIVec");
		...
		int i;
		int * tmp;
		tmp = zMalloc(vec->limit * sizeof(int), "zPushIVecNew");
...
void zPushFVec (zFVec *vec, float val) {
	...
	//	vec->elem = zRealloc(vec->elem, vec->limit * sizeof(float), "zPushFVec");
		...
		int i;
		float * tmp;
		tmp = zMalloc(vec->limit * sizeof(float), "zPushFVecNew");
		for (i = 0; i < vec->size; ++i)
		{
			tmp[i]=vec->elem[i];
		}
...
void zPushTVec (zTVec* vec, const char* text) {
	...
	//	vec->elem = zRealloc(vec->elem, vec->limit * sizeof(char*), "zPushTVec");
		...
		int i;
		char ** tmp;
		tmp = zMalloc(vec->limit * sizeof(char), "zPushTVecNew");
		for (i = 0; i < vec->size; ++i)
		{
			tmp[i] = zMalloc(strlen(vec->elem[i]) +1, "zPushTVec");
			strcpy(tmp[i],vec->elem[i]);
...
void zPushVec (zVec *vec, void *thing) {
	...
	//	vec->elem = zRealloc(vec->elem, vec->limit * sizeof(void*), "zPushVec");
		...
		int i;
		void ** tmp;
		tmp = zMalloc(vec->limit * sizeof(void*), "zPushVecNew");
		for (i = 0; i < vec->size; ++i)
		{
			tmp[i]=vec->elem[i];
		}
